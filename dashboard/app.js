...
function _digestName(t) { return _journalTypeNames[t] || t; }
...
function _dgCard(title, note, body) {
  return `<div style="border:1px solid #222;border-radius:6px;margin:8px 10px;overflow:hidden">
    <div style="padding:6px 10px;background:#181818;border-bottom:1px solid #222;display:flex;align-items:baseline;gap:8px">
      ...
      ${note ? `<span style="color:#666;font-size:10px">${escHtml(note)}</span>` : ''}
    </div>
...
function renderDigest(data) {
  ...
  if (!dg) {
    body = data.status === 'error'
      ...
      : `<div style="padding:20px;color:#666;font-size:12px">还没有速递——点击「生成今日速递」聚合：大盘环境、最近新增信号、历史战绩回顾、核心池全量扫描与历史统计摘要。</div>`;
  } else {
    body = _digestSections(dg);
  }
  const foot = `
    <div style="padding:6px 12px 8px;font-size:10px;color:#666;line-height:1.6">
...
function _digestSections(dg) {
  ...
  } else marketHtml = '<span style="color:#666">大盘环境暂不可用</span>';
  ...
  if (!recentHtml && (rs.groups || []).length === 0) recentHtml = `<div style="padding:10px;color:#666">最近没有新增信号——产生买卖信号后会自动落档。</div>`;
  if (!recentHtml) {
  ...
  let maturedHtml;
  if (matured.length === 0) maturedHtml = `<div style="padding:10px;color:#666">窗口内没有到期的信号战绩（生成前已自动补记）</div>`;
  else {
  ...
  if (!scanHtml && ps.total === 0) scanHtml = `<div style="padding:10px;color:#666">核心池为空——先把股票加入核心池再生成速递。</div>`;
  if (!scanHtml) {
    const mkRow = r => {
      const nm = _knownName(r.symbol);
      return `<tr>
    ...
    scanHtml = `
      ...
      ${(ps.buy || []).length ? `<table style="width:100%;border-collapse:collapse;font-size:11px;color:#ccc"><thead>${head}</thead><tbody>${buyRows}</tbody></table>` : '<div style="padding:10px;color:#666">今日核心池无买入信号</div>'}
