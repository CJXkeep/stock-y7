...
个人自用的 A 股趋势分析工具：多周期 K 线、五模块信号引擎与缠论买卖点，本地 Web 看板实时查看；v5 增加信号日志（SQLite 信号档案）、核心池管理、历史信号统计管线与「评估 → 响应闭环」。纯 Python 标准库实现（无第三方运行时依赖），主线已推进到 **I9（选股层与滚动评估，2026-09-01 落地）**，详见 `docs/版本路线图.md` 与 `docs/i9/选股层与滚动评估设计.md`。
