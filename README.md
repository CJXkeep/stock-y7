...
2. 交易时段内按 `interval_min`（默认 15 分钟）自动巡检：**持仓巡检（每轮）** +
   全市场选股（受 `screening_interval_min` 节流，默认 60 分钟）；
3. 也可用「立即执行一轮」手动触发（跳过交易时段限制，便于验证）。

### 口径要点

- **选股**：全 A 快照剔 ST / 退市 / 停牌 → 按成交额取前 `scan_limit`（默认 300）→
  日 K 初筛（本地 K 线库，零逐股行情请求）→ 命中买入档位的候选补拉资金流重算 →
