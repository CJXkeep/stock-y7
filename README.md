...
给出账户概览与组合级绩效（年化 / 最大回撤 / 夏普 / 卡玛）。设计稿 `docs/迭代_v6-v8_模拟账户/模拟账户设计.md`。
「贡献拆解」卡可按**标的 / 档位**下钻已实现 + 浮动盈亏的归因（纯披露不设门槛），
设计稿 `docs/迭代_i11_持仓归因/持仓级净值归因设计.md`。
...
不是组合账户收益；设计详见 `docs/迭代_i8_评估闭环/评估模块设计.md` 与 `docs/迭代_i8_评估闭环/信号响应闭环设计.md`，
首次实盘基线见 `docs/迭代_i8_评估闭环/评估基线-2026-08-29.md`。
...
I9 把"入池"变成**数据支撑 + 留痕**的管线；组合模拟押后至 v6 条件项。设计稿：`docs/迭代_i9_选股层/选股层与滚动评估设计.md`。
